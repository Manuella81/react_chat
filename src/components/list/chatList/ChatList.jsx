...
const ChatList = () => {
  ...
  const { chatId, changeChat } = useChatStore(); // Utilisez chatId et changeChat du store
  ...
  const handleSelect = async (chat) => {
      ...
      try {
          ...
          changeChat(chat.chatId, chat.user); // Met à jour chatId dans le store
  ...
  // Filtre du tableau chats
  const filteredChats = chats.filter((c) =>
      c.user.username.toLowerCase().includes(input.toLowerCase())
  );

  return (
      <div className={`chatList ${chatId ? 'hide-on-conversation' : ''}`}>
